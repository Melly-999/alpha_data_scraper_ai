...
# Used by lstm_signal_adapter.py and external consumers that need individual
# indicator series rather than the all-in-one add_indicators() DataFrame.


def compute_atr(
    high: pd.Series, low: pd.Series, close: pd.Series, period: int = 14
) -> pd.Series:
    """Public API: Average True Range."""
...
def compute_atr_pct(
    ...
    """Public API: ATR as percentage of close."""
    atr = _atr(high, low, close, period)
...
def compute_adx(
    ...
    """Public API: (ADX, +DI, -DI)."""
    return _adx(high, low, close, period)


def compute_obv_zscore(
    close: pd.Series, volume: pd.Series, window: int = 50
) -> pd.Series:
    """Public API: On-Balance-Volume rolling z-score."""
    obv_raw = _obv(close, volume)
    obv_mean = obv_raw.rolling(window, min_periods=10).mean()
    obv_std = obv_raw.rolling(window, min_periods=10).std()
    return (obv_raw - obv_mean) / (obv_std + 1e-12)


def compute_vwap_deviation(
    high: pd.Series, low: pd.Series, close: pd.Series, volume: pd.Series
) -> pd.Series:
    """Public API: (close − VWAP) / VWAP × 100."""
    vwap = _vwap(high, low, close, volume)
    return (close - vwap) / (vwap.abs() + 1e-12) * 100


@dataclass
class RollingIndicatorBuffer:
    """Bounded OHLCV cache for incremental indicator updates."""

    maxlen: int = DEFAULT_ROLLING_WINDOW_SIZE
    _rows: deque[dict[str, Any]] = field(init=False, repr=False)
    _seen_times: set[Any] = field(init=False, repr=False)
    _time_order: deque[Any] = field(init=False, repr=False)
    _last_time: Any = field(default=None, init=False, repr=False)

    def __post_init__(self) -> None:
        if self.maxlen < 60:
            raise ValueError("RollingIndicatorBuffer maxlen must be at least 60")
        self._rows = deque(maxlen=self.maxlen)
        self._seen_times = set()
        self._time_order = deque(maxlen=self.maxlen)

    def append(self, candle: pd.Series | dict[str, Any]) -> pd.Series | None:
        row = candle.to_dict() if isinstance(candle, pd.Series) else dict(candle)
        candle_time = row.get("time")
        if candle_time is not None and candle_time in self._seen_times:
            return None

        if candle_time is not None and len(self._time_order) == self.maxlen:
            oldest_time = self._time_order.popleft()
            self._seen_times.discard(oldest_time)

        self._rows.append(row)
        if candle_time is not None:
            self._seen_times.add(candle_time)
            self._time_order.append(candle_time)
            self._last_time = candle_time

        if len(self._rows) < 2:
            return None

        enriched = add_indicators(self.frame)
        if enriched.empty:
            return None
        return enriched.iloc[-1]

    def extend(self, candles: pd.DataFrame) -> pd.DataFrame:
        latest_rows: list[pd.Series] = []
        for _, candle in candles.iterrows():
            latest = self.append(candle)
