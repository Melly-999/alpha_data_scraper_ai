...
MID_COMMERCIAL_JOB = {
    ...
    "must_have_requirements": [
        "Python", "FastAPI", "PostgreSQL", "commercial IT experience required",
    ],
...
STACKED_PENALTY_JOB = {
    ...
    "matura_requirement": "none",
    "must_have_requirements": ["Python", "PostgreSQL", "Apache Spark", "5+ years experience"],
...
def test_sm_03_junior_internship_role_can_score_apply_now():
    ...
    assert result["tag"] in ("stretch", "apply_now"), (
...
def test_sm_07_team_leadership_required_no_leadership_claim():
    ...
    assert any(
        "manage" in g or "team" in g or "hiring" in g
    ...
    for false_claim in (
        ...
        assert false_claim not in output, (
...
def test_sm_08_production_ownership_required_no_ownership_claim():
    ...
    blocking_lower = [g.lower() for g in result["missing_skills"]["blocking"]]
    assert any(
        "production" in g or "own" in g or "kubernetes" in g
        for g in blocking_lower
    ), (
...
def test_sm_09_degree_plus_seniority_stacks_penalties():
    ...
    assert total <= -55, (
        f"Expected stacked penalty ≤ -55, got {total}"
    )
