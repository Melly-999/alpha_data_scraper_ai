...
def test_nf_02_matura_required_fires_penalty_no_false_claim():
    ...
    # Penalty must fire
    assert (
        "mandatory_matura_if_not_confirmed" in result["penalties"]
...
def test_nf_05_production_fintech_no_production_ownership_claim():
    ...
    assert any(
        "production" in g or "commercial" in g or "2 years" in g for g in blocking_lower
    ...
    for false_claim in (
        ...
        assert (
            false_claim not in evidence_text
...
def test_nf_08_guaranteed_trading_results_is_hard_skipped():
    ...
    assert result["score"] <= 24, f"Hard skip score must be ≤ 24, got {result['score']}"
    assert result["tag"] == "skip"
...
def test_nf_10_suggested_portfolio_tasks_are_learning_tasks_not_fabrication():
    ...
    assert isinstance(tasks, dict), "suggested_next_portfolio_tasks must be a dict"
