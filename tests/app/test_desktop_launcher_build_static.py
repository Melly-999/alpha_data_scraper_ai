...
BUILD_VALIDATION_DOC = REPO_ROOT / "docs" / "qa" / "desktop_launcher_build_validation.md"
...
class TestBuildValidationDocExists:
    def test_doc_exists(self) -> None:
        assert BUILD_VALIDATION_DOC.is_file(), (
...
class TestBuildValidationDocContent:
    ...
    def test_references_exe(self, doc_text: str) -> None:
        assert "dist/MellyTradeLauncher.exe" in doc_text, (
    ...
    def test_must_not_be_committed_wording(self, doc_text: str) -> None:
        ...
        assert "must not be committed" in lower or "must not commit" in lower or "not be committed" in lower, (
...
class TestBuildWrapperBuildMode:
    ...
    def test_has_build_parameter(self, ps1_text: str) -> None:
        assert "[switch]$Build" in ps1_text, (
...
class TestNoGeneratedArtifactsTracked:
    ...
    def test_dist_exe_not_tracked(self, tracked_files: set[str]) -> None:
        ...
        assert "dist/MellyTradeLauncher.exe" not in tracked_files, (
    ...
    def test_no_exe_in_dist_tracked(self, tracked_files: set[str]) -> None:
        ...
        tracked_exes = [f for f in tracked_files if f.startswith("dist/") and f.endswith(".exe")]
        assert tracked_exes == [], (
