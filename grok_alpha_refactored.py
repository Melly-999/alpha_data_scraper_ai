...
class GrokAlphaGUI:
    ...
    def update_data(self):
        try:
            ...
            _, new_df = self.mt5_fetcher.get_new_bars()
            ...
            confidence = self.signal_gen.compute_confidence(
                slope, rsi, stoch_k, lstm_dir, bb_pos
            )

            if signal and confidence >= self.conf_threshold:
                log_msg = (
                    f"[{datetime.now().strftime('%H:%M:%S')}] [AI] {signal} | "
                    f"Conf:{confidence}% | LSTM+RSI:{lstm_dir} | "
                    f"StochK:{stoch_k:.1f} | {hold_mode}"
                )
...
TODAY PROFIT: +{profit:.2f} USD (Goal 15.00)
        ...
        self.status_label.config(
            text=f"{'🟢' if confidence >= self.conf_threshold else '🔴'} "
            f"Sygnał: {signal or 'BRAK'} | Dynamic Holding Time: {hold_mode}",
