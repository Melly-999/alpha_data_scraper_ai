...
def detect_market_regime(
    ...
    """Adapter-compatible regime detection used by lstm_signal_adapter.

    Accepts keyword-only style from the adapter (adx + atr_pct).
    High ATR% (>3%) with low ADX maps to RANGING (volatile but trendless).
    """
    if adx < 20:
        return MarketRegime.RANGING
    return (
