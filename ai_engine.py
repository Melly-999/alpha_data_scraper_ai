...
class AIEngine:
    ...
    def analyze_all(self) -> Dict[str, UnifiedSignal]:
        ...
        try:
            ...
            return asyncio.run(self.analyze_all_async())
        # Fallback for sync callers when loop is already running
        return self._analyze_all_sync()

    async def analyze_all_async(self) -> Dict[str, UnifiedSignal]:
        """Async version — runs data fetch (CPU-bound, in thread) and
        sentiment fetch (I/O-bound, async) concurrently via asyncio.gather.
        """
        loop = asyncio.get_running_loop()

        # 1. Data fetch is synchronous (MT5 C library) — offload to thread
        ...
        # 2. Sentiment is async — start concurrently
        sentiment_future: asyncio.Task[Dict[str, SentimentScore]] | None = None
        if self.sentiment_analyzer:
            sentiment_future = asyncio.create_task(
                self.sentiment_analyzer.analyze_sentiment_async(
        ...
        if sentiment_future:
            try:
                sentiment_map = await sentiment_future
            except Exception as e:
                logger.warning(f"Sentiment analysis failed: {e}")

        # 3. Analyze each symbol (CPU-bound, sequential — fast)
    ...
    def _analyze_all_sync(self) -> Dict[str, UnifiedSignal]:
        """Synchronous fallback for callers already inside an event loop."""
        signals: Dict[str, UnifiedSignal] = {}
        try:
