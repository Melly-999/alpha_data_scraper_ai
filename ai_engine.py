...
class AlphaAIEngine:
    ...
    def analyze_and_decide(self, symbols: list = None) -> dict:
        ...
        try:
            ...
            logger.info(f"Dywidendy netto: {dividend_analysis.get('net_dividends', 0):.2f} PLN")
        except:
            dividend_analysis = {}
        ...
        return {
            ...
            "risk_report_summary": risk_report[:500] + "..." if len(risk_report) > 500 else risk_report,
    ...
    def _build_portfolio_string(self) -> str:
        ...
        for p in positions[:15]:
            parts.append(f"{p['symbol']} {p['qty']:.2f} szt. (avg {p.get('avg_cost', 0):.2f})")
        return " | ".join(parts)
