...
class XTBBroker(BrokerInterface):
    ...
    def __init__(
        self,
        closed_positions_xlsx_paths: Optional[Union[str, list[str]]] = None,
        cash_operations_xlsx_paths: Optional[Union[str, list[str]]] = None,
        default_currency: str = "PLN",
    ):
        self.closed_positions_paths = (
            [closed_positions_xlsx_paths]
            if isinstance(closed_positions_xlsx_paths, str)
            else (closed_positions_xlsx_paths or [])
        )
        self.cash_operations_paths = (
            [cash_operations_xlsx_paths]
            if isinstance(cash_operations_xlsx_paths, str)
            else (cash_operations_xlsx_paths or [])
        )
        self.default_currency = default_currency
        self._positions_cache: Optional[list[dict[str, Any]]] = None
        self._cash_cache: Optional[list[dict[str, Any]]] = None
    ...
    def _load_closed_positions(self) -> List[Dict]:
        ...
        for path in self.closed_positions_paths:
            ...
            try:
                ...
                logger.info(
                    f"✅ Wczytano {len(all_positions)} pozycji z {os.path.basename(path)}"
                )
    ...
    def get_historical_data(
        ...
        if yf is None:
            logger.warning(
                "yfinance unavailable; returning empty history for %s", symbol
            )
            return pd.DataFrame()
    ...
    def get_cash_summary(self) -> Dict:
        ...
        summary = {
            ...
            "total_dividends_gross": float(
                df[df["type"].str.contains("Dividend", case=False, na=False)][
                    "amount"
                ].sum()
            ),
            "withholding_tax": float(
                df[
                    df["type"].str.contains("Withholding tax|tax", case=False, na=False)
                ]["amount"].sum()
            ),
            "net_dividends": 0.0,
            "deposits": float(
                df[
                    df["type"].str.contains(
                        "Deposit|IKE deposit|Transfer in", case=False, na=False
                    )
                ]["amount"].sum()
            ),
            "withdrawals": float(
                df[df["type"].str.contains("Withdrawal", case=False, na=False)][
                    "amount"
                ].sum()
            ),
            "top_dividend_stocks": df[
        ...
        summary["net_dividends"] = (
            summary["total_dividends_gross"] + summary["withholding_tax"]
        )
        return summary
