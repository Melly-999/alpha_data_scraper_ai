...
try {
    ...
    Invoke-SetupCommand "pip upgrade" { & $venvPython -m pip install --upgrade pip }

    if ($installDeps) {
        Invoke-SetupCommand "CI/dev requirements install" {
            & $venvPython -m pip install -r requirements-ci.txt
        }
        Invoke-SetupCommand "runtime requirements install" {
    ...
    Write-Host "Jakosc: .\run_quality.ps1"
}
finally {
    Pop-Location
}
