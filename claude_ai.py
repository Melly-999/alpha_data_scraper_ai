...
class ClaudeAIClient:
    ...
    def get_trading_signal(self, market_data: Dict[str, Any]) -> Optional[ClaudeSignal]:
        return _run_async(self.get_trading_signal_async(market_data))

    async def get_trading_signal_async(
    ...
    def get_portfolio_signal(
        ...
        return _run_async(self.get_portfolio_signal_async(portfolio_data))

    async def get_portfolio_signal_async(
        self, portfolio_data: Dict[str, Dict]
    ) -> Optional[Dict[str, ClaudeSignal]]:
        """
        ...
        """
        tasks = {
            symbol: self.get_trading_signal_async(data)
            for symbol, data in portfolio_data.items()
        }
        results = await asyncio.gather(*tasks.values(), return_exceptions=True)

        signals: dict[str, ClaudeSignal] = {}
        for symbol, sig in zip(tasks.keys(), results):
            if isinstance(sig, BaseException):
                logger.warning(f"Claude signal failed for {symbol}: {sig}")
                continue
            if sig:
                signals[symbol] = sig
        return signals
...
- confidence + risk must be balanced (high conf + high risk = aggressive)"""
    ...
    async def _call_api_async(self, prompt: str) -> Optional[str]:
...
class ClaudeAIIntegration:
    ...
    async def validate_signal_async(
        ...
        try:
            ...
            claude_sig = await self.client.get_trading_signal_async(market_data)
            if not claude_sig:
    ...
    def get_independent_signal(
        ...
        return _run_async(self.get_independent_signal_async(market_data))

    async def get_independent_signal_async(
        self, market_data: Dict[str, Any]
    ) -> Optional[ClaudeSignal]:
        ...
        try:
            return await self.client.get_trading_signal_async(market_data)
        except Exception as e:
